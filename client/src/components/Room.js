...
function Room({ initialState, socketId, onLeave }) {
  ...
  const [gameState, setGameState] = useState(
    initialState.gameState || "WAITING",
  );
  ...
  useEffect(() => {
    ...
    const onPlayerJoined = ({ username, avatar }) =>
      setToasts((prev) => [
        ...prev,
        { id: Date.now(), type: "join", message: `${username} joined`, avatar },
      ]);
    const onPlayerLeft = ({ username, avatar }) =>
      setToasts((prev) => [
        ...prev,
        { id: Date.now(), type: "leave", message: `${username} left`, avatar },
      ]);
  ...
  const handleInvite = useCallback(() => {
    ...
    if (navigator.share && navigator.canShare) {
      navigator
        .share({ title: "Dooduel", text: `Join room: ${roomId}`, url })
        .catch(() => {});
    } else {
  ...
  const diffBadgeColor =
    { Easy: "bg-pixel-green", Medium: "bg-pixel-gold", Hard: "bg-pixel-red" }[
      difficulty
    ] || "bg-pixel-dim";
  ...
  if (gameState === "GAME_OVER" && gameSummary) {
    return (
      <div className="flex flex-col flex-1 overflow-hidden">
        {/* Header */}
        <header
          className="relative h-14 bg-pixel-bgdark border-b-4 border-pixel-border flex items-center justify-between px-4 flex-shrink-0"
          style={{ boxShadow: "0 4px 0 #000", zIndex: 10 }}
        >
          <span
            className="font-pixel text-sm text-pixel-gold"
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden relative pb-12 lg:pb-0">
          ...
          >
            {toast.avatar &&
              (toast.avatar.url ? (
                ...
                <span
                  className="w-5 h-5 border-2 border-pixel-border flex items-center justify-center text-sm flex-shrink-0"
      ...
      {/* Floating reactions */}
      <div
        className="fixed bottom-16 left-1/2 pointer-events-none"
        style={{ zIndex: 40 }}
      >
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        ...
        <section className="flex-1 flex flex-col overflow-hidden">
              ...
              {Object.entries(players).map(([id, player]) => (
                ...
                >
                  {player.avatar?.url ? (
                    ...
                    <span
                      className="w-5 h-5 flex items-center justify-center text-xs border border-pixel-border flex-shrink-0"
                      style={{
                        backgroundColor: player.avatar?.color || "#444",
                      }}
                    >
                      {player.avatar?.emoji || "👤"}
                    </span>
                  ...
                  <div className="flex flex-col min-w-0">
                    <span
                      className={`font-pixel text-[5px] leading-tight ${id === socketId ? "text-pixel-cyan" : id === host ? "text-pixel-gold" : "text-white"}`}
                    >
                      {player.username.slice(0, 8)}
                    </span>
                    <span className="font-pixel text-[5px] text-pixel-gold leading-tight">
                      {player.score ?? 0}pt
                    </span>
                  </div>
                </div>
          ...
          {error && (
            <div
              className="bg-pixel-red border-4 border-pixel-border font-pixel text-[10px] text-white px-3 py-2 m-2 flex-shrink-0"
          ...
          <div
            className="flex-1 overflow-y-auto animate-phase-slide"
            key={phaseKey}
          >
            {/* WAITING */}
            {gameState === "WAITING" && (
              <div className="flex flex-col items-center gap-6 p-6">
                <div
                  className="font-pixel text-sm text-pixel-gold text-center"
                  style={{ textShadow: "2px 2px 0 #000" }}
                >
                  WAITING FOR PLAYERS...
                </div>
                ...
                <div className="flex gap-3 flex-wrap justify-center">
                  {[
                    ...
                    <div key={i} className="pixel-card px-4 py-3 text-center">
                      <div
                        className={`font-pixel text-[8px] text-pixel-black border-2 border-pixel-border px-2 py-0.5 mb-2 ${r.color}`}
                      >
                        {r.label}
                      </div>
                      <div className="font-pixel text-[8px] text-pixel-dim">
                        {r.time}
                      </div>
                    </div>
                  ))}
                ...
                <button
                  className="pixel-btn-secondary text-[10px] px-4 py-2"
            ...
            {gameState === "DRAWING" && (
              <div className="flex flex-col h-full">
                ...
                <div className="flex items-center gap-3 bg-pixel-bgdark border-b-4 border-pixel-border px-4 py-3 border-l-8 border-l-pixel-gold flex-shrink-0">
                  ...
                  {difficulty && (
                    <span
                      className={`font-pixel text-[8px] py-2 text-pixel-black border-2 border-pixel-border px-2 whitespace-nowrap ${diffBadgeColor}`}
                    >
