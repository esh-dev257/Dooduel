...
function Chat({ roomId, socketId, players }) {
  ...
  useEffect(() => {
    const onChatMessage = (message) => {
      ...
      if (mobile ? !mobileOpenRef.current : isMinimized) setUnreadCount(c => c + 1);
    };
  ...
  return (
    <>
      {/* ===== DESKTOP: side panel — identical to original, just hidden on mobile ===== */}
      <div className={`hidden lg:flex flex-col bg-pixel-panel border-l-4 border-pixel-border overflow-hidden flex-shrink-0 transition-[width] duration-200 ${isMinimized ? 'w-8' : 'w-[260px]'}`}>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 flex flex-col bg-pixel-panel border-t-4 border-pixel-border" style={{ boxShadow: '0 -4px 0 #000' }}>

        {/* Expanded: messages + input */}
        {mobileOpen && (
          <>
            <div className="overflow-y-auto flex flex-col gap-1 p-2 chat-msg-area" style={{ height: '45vh' }}>
              {messages.length === 0 && (
              ...
              {messages.map((msg, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`flex flex-col max-w-[95%] border-2 px-2 py-1
                      ${isSelf ? 'self-end border-pixel-cyan bg-pixel-bgdark' : 'self-start border-pixel-borderAlt bg-pixel-bgdark'}`}
                    style={{ boxShadow: '2px 2px 0 #000' }}
                  >
                    <div className="flex items-center gap-1 mb-0.5">
                      {msg.avatar?.url
                        ? <img src={msg.avatar.url} alt="avatar" className="w-4 h-4 border border-pixel-border flex-shrink-0" style={{ imageRendering: 'pixelated' }} />
                        : <span className="w-4 h-4 border border-pixel-border flex items-center justify-center text-xs flex-shrink-0" style={{ backgroundColor: msg.avatar?.color || '#444' }}>{msg.avatar?.emoji || ''}</span>
                      }
                      <span className={`font-pixel text-[7px] truncate flex-1 ${isSelf ? 'text-pixel-cyan' : 'text-pixel-gold'}`}>
                        {isSelf ? 'YOU' : msg.username}
                      </span>
                    </div>
                    <span className="font-pixel text-[8px] text-white break-words leading-relaxed">{msg.text}</span>
                  </div>
                );
              })}
        ...
        <div
          className="h-12 flex items-center justify-between px-4 cursor-pointer select-none flex-shrink-0 bg-pixel-bgdark"
          onClick={mobileOpen ? closeMobile : openMobile}
        >
          <span className="font-pixel text-[9px] text-pixel-gold"> CHAT</span>
          {!mobileOpen && unreadCount > 0 && (
            <div className="bg-pixel-red border-2 border-pixel-border w-5 h-5 flex items-center justify-center" style={{ boxShadow: '2px 2px 0 #000' }}>
              <span className="font-pixel text-[6px] text-white leading-none">{unreadCount > 9 ? '9+' : unreadCount}</span>
            </div>
          )}
          <span className="font-pixel text-[8px] text-pixel-dim">{mobileOpen ? '▼' : '▲'}</span>
        </div>
      </div>
    </>
  );
